...
phi_h3(phi_h3>180) = phi_h3(phi_h3>180) - 360;
phi_h4(phi_h4>180) = phi_h4(phi_h4>180) - 360;
...
for wn = 1:numel(whale)
    ...
    in2km = find(R<2000 & wloc(:, 3)<300);
    if length(in2km)<300 % if whale is never within 2km, skip to next whale
        continue
    end
    if (max(t(in2km))-min(t(in2km)))<600 % if time in 2km zone is less than 300 s (5 min) skip to next whale
        continue 
